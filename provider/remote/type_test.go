...
func TestTypeInterface(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			typeImpl:     &TypeImpl{Name: "bool", Value: model.TypeBooleanValue},
			initVal:      true,
			expectedType: true,
			expectError:  false,
		},
		{
			...
			typeImpl:     &TypeImpl{Name: "int", Value: model.TypeIntegerValue},
			initVal:      123,
		...
		},
		{
			...
			typeImpl:     &TypeImpl{Name: "float64", Value: model.TypeFloatValue},
		...
		},
		{
			...
			typeImpl:     &TypeImpl{Name: "string", Value: model.TypeStringValue},
			initVal:      "test",
			expectedType: "test",
			expectError:  false,
		},
		{
			...
			typeImpl:     &TypeImpl{Name: "time.Time", Value: model.TypeDateTimeValue},
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			if test.expectedErr {
				...
				if err != nil {
					t.Errorf("Got unexpected error for %v: %v", test.value, err)
					return
				}
				...
				if typeInfo.GetValue() == 0 {
					t.Errorf("Expected valid type value, got 0")
				}
			}
		})
	}
