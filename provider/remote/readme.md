...
3. SpecImpl->ViewDeclare 表示当前Field值对应的视图声明类型。运行时内部仍可能出现 OriginView/MetaView/DetailView/LiteView，但对外稳定的 tag 输入只有 DetailView 和 LiteView；未声明 tag 时，运行时按 OriginView 处理

4. SpecImpl->DefaultValue 表示当前Field值对应的默认值，如果为空表示当前Field值没有默认值，否则则使用该值为默认值，在模型对象值对应的Field值为为空时，使用该值为默认值

5. 子对象包含/引用查询层级不是 SpecImpl 的公开配置项。默认行为由框架内置规则决定：顶层对象按自身视图裁剪，子对象统一收敛到 LiteView。
