...
func (s *impl) insertSingleRelation(vModel model.Model, vField model.Field) (ret model.Value, err *cd.Result) {
	...
	entityVal, entityErr := s.modelProvider.GetEntityValue(rModel.Interface(fType.IsPtrType(), 0))
	if entityErr != nil {
		err = entityErr
...
func (s *impl) insertSliceRelation(vModel model.Model, vField model.Field) (ret model.Value, err *cd.Result) {
	...
	for _, fVal := range fSliceValue {
		...
		entityVal, entityErr := s.modelProvider.GetEntityValue(rModel.Interface(elemType.IsPtrType(), 0))
		if entityErr != nil {
			err = entityErr
