...
func testEmptyObject(t *testing.T, o1 orm.Orm, localProvider provider.Provider) {
	...
	emptyObj = emptyObjModel.Interface(true, model.OriginView).(*EmptyObject)
...
func testCircularReference(t *testing.T, o1 orm.Orm, localProvider provider.Provider) {
	...
	parentObj = parentObjModel.Interface(true, model.OriginView).(*CircularObject)
	...
	childObj = childObjModel.Interface(true, model.OriginView).(*CircularObject)
	...
	parentObj = updatedParentObjModel.Interface(true, model.OriginView).(*CircularObject)
	...
	checkChild = checkChildModel.Interface(true, model.OriginView).(*CircularObject)
	...
	// 指定完整查询，确保获取关联的子对象
	filter, filterErr := localProvider.GetModelFilter(queryParentObjModel, model.OriginView)
	if filterErr != nil {
	...
	queryParentObj = parentModelList[0].Interface(true, model.OriginView).(*CircularObject)
...
func testMaxSizeObject(t *testing.T, o1 orm.Orm, localProvider provider.Provider) {
	...
	maxObj = maxObjModel.Interface(true, model.OriginView).(*MaxSizeObject)
	...
	// 使用批量查询以确保获取完整数据
	maxObjFilter, filterErr := localProvider.GetModelFilter(queryMaxObjModel, model.OriginView)
	if filterErr != nil {
		t.Errorf("GetModelFilter failed, err:%s", filterErr.Error())
		return
	}
	...
	queryMaxObj = maxObjList[0].Interface(true, model.OriginView).(*MaxSizeObject)
...
func TestCircularReference(t *testing.T) {
	...
	queryParent = queryParentModel.Interface(true, model.OriginView).(*CircularObject)
...
func TestMaxSizeObject(t *testing.T) {
	...
	queryObj = queryModel.Interface(true, model.OriginView).(*MaxSizeObject)
