...
func testBatchInsert(t *testing.T, o1 orm.Orm, localProvider provider.Provider) {
	...
	filter, err := localProvider.GetModelFilter(batchItemModel, model.OriginView)
	if err != nil {
...
func testBatchQuery(t *testing.T, o1 orm.Orm, localProvider provider.Provider) {
	...
	equalFilter, err := localProvider.GetModelFilter(batchItemModel, model.OriginView)
	if err != nil {
		t.Errorf("GetModelFilter failed, err:%s", err.Error())
		return
	}
	...
	rangeFilter, err := localProvider.GetModelFilter(batchItemModel, model.OriginView)
	if err != nil {
		t.Errorf("GetModelFilter failed, err:%s", err.Error())
		return
	}
	...
	likeFilter, err := localProvider.GetModelFilter(batchItemModel, model.OriginView)
	if err != nil {
		t.Errorf("GetModelFilter failed, err:%s", err.Error())
		return
	}
	...
	combinedFilter, err := localProvider.GetModelFilter(batchItemModel, model.OriginView)
	if err != nil {
		t.Errorf("GetModelFilter failed, err:%s", err.Error())
		return
	}
	...
	pageFilter, err := localProvider.GetModelFilter(batchItemModel, model.OriginView)
	if err != nil {
		t.Errorf("GetModelFilter failed, err:%s", err.Error())
		return
	}
...
func testBatchUpdate(t *testing.T, o1 orm.Orm, localProvider provider.Provider) {
	...
	for i, itemModel := range modelList {
		item := itemModel.Interface(true, model.OriginView).(*BatchItem)
	...
	// 验证更新结果
	updatedFilter, err := localProvider.GetModelFilter(batchItemModel, model.OriginView)
	if err != nil {
		t.Errorf("GetModelFilter failed, err:%s", err.Error())
		return
	}
...
func testBatchDelete(t *testing.T, o1 orm.Orm, localProvider provider.Provider) {
	...
	// 验证删除结果
	verifyFilter, err := localProvider.GetModelFilter(batchItemModel, model.OriginView)
	if err != nil {
		t.Errorf("GetModelFilter failed, err:%s", err.Error())
		return
	}
