...
func testComplexNesting(t *testing.T, o1 orm.Orm, localProvider provider.Provider) {
	...
	level1Filter, err := localProvider.GetModelFilter(level1ItemModel, model.OriginView)
	if err != nil {
	...
	if len(level1ModelList) == 0 {
		...
		level3Item = *l3Model.Interface(true, model.OriginView).(*Level3Item)
		...
		level2Item = *l2Model.Interface(true, model.OriginView).(*Level2Item)
		...
		if l1Model != nil {
			level1Item = l1Model.Interface(true, model.OriginView).(*Level1Item)
		} else {
			t.Log("l1Model is nil after insertion")
			return
		}
	} else if len(level1ModelList) > 0 && level1ModelList[0] != nil {
		level1Item = level1ModelList[0].Interface(true, model.OriginView).(*Level1Item)
	} else {
	...
	// 查询是否已存在 Extra_L1 对象
	extraL1Filter, _ := localProvider.GetModelFilter(level1ItemModel, model.OriginView)
	...
	if len(extraL1ModelList) == 0 {
		...
		if extraL1Model != nil {
			extraL1Item = extraL1Model.Interface(true, model.OriginView).(*Level1Item)
		} else {
			t.Log("extraL1Model is nil after insertion")
			return
		}
	} else if len(extraL1ModelList) > 0 && extraL1ModelList[0] != nil {
		extraL1Item = extraL1ModelList[0].Interface(true, model.OriginView).(*Level1Item)
	} else {
	...
	complexFilter, complexErr := localProvider.GetModelFilter(complexModel, model.OriginView)
	if complexErr != nil {
	...
	if len(complexModelList) == 0 {
		...
		complexItem = *complexModel.Interface(true, model.OriginView).(*ComplexNestedItem)
	} else {
		complexItem = *complexModelList[0].Interface(true, model.OriginView).(*ComplexNestedItem)
	}
	...
	queriedComplex := queryComplexModel.Interface(true, model.OriginView).(*ComplexNestedItem)
...
func cleanupNestedTest(t *testing.T, o1 orm.Orm, localProvider provider.Provider) {
	...
	complexModel, _ := localProvider.GetEntityModel(&ComplexNestedItem{})
	complexFilter, err := localProvider.GetModelFilter(complexModel, model.OriginView)
	if err != nil {
