...
func TestLocalTransaction(t *testing.T) {
	...
	obj1 = obj1Model.Interface(true, model.OriginView).(*Unit)
	...
	queryObj1 = queryObj1Model.Interface(true, model.OriginView).(*Unit)
	...
	checkObj1 = checkObj1Model.Interface(true, model.OriginView).(*Unit)
...
func TestLocalBatchOperation(t *testing.T) {
	...
	for idx := 0; idx < batchSize; idx++ {
		...
		unitList[idx] = unitModel.Interface(true, model.OriginView).(*Unit)
	}

	// 使用过滤器批量查询
	unitModel, _ := localProvider.GetEntityModel(&Unit{})
	filter, err := localProvider.GetModelFilter(unitModel, model.OriginView)
	if err != nil {
	...
	// 使用LIKE查询
	filter2, err := localProvider.GetModelFilter(unitModel, model.OriginView)
	if err != nil {
		t.Errorf("GetEntityFilter failed, err:%s", err.Error())
		return
	}
	...
	// 分页查询测试
	filter3, err := localProvider.GetModelFilter(unitModel, model.OriginView)
	if err != nil {
		t.Errorf("GetEntityFilter failed, err:%s", err.Error())
		return
	}
	...
	// 排序查询测试
	filter4, err := localProvider.GetModelFilter(unitModel, model.OriginView)
	if err != nil {
		t.Errorf("GetEntityFilter failed, err:%s", err.Error())
		return
	}
	...
	for i := 0; i < len(bq4ModelList)-1; i++ {
		unit1 := bq4ModelList[i].Interface(true, model.OriginView).(*Unit)
		unit2 := bq4ModelList[i+1].Interface(true, model.OriginView).(*Unit)
