...
func benchmarkBulkQuerySimple(t *testing.T, o1 orm.Orm, localProvider provider.Provider) {
	...
	filter, err := localProvider.GetModelFilter(simplePerfModel, model.OriginView)
	if err != nil {
...
func benchmarkInsertComplex(t *testing.T, o1 orm.Orm, localProvider provider.Provider, count int) {
	...
	for i := 0; i < 10; i++ {
		...
		simpleObjList = append(simpleObjList, simpleObjModel.Interface(true, model.OriginView).(*SimplePerf))
	}
...
func benchmarkQueryComplex(t *testing.T, o1 orm.Orm, localProvider provider.Provider) {
	...
	filter, err := localProvider.GetModelFilter(complexPerfModel, model.OriginView)
	if err != nil {
		t.Errorf("GetModelFilter failed, err:%s", err.Error())
		return
	}
	...
	// 测试单个复杂对象的查询和序列化性能
	if count > 0 {
		complexObj := modelList[0].Interface(true, model.OriginView).(*ComplexPerf)
		
		startDeserializeTime := time.Now()
		
		// 测试反序列化性能
		for i := 0; i < 100; i++ {
			_ = modelList[i % count].Interface(true, model.OriginView).(*ComplexPerf)
		}
...
func benchmarkFilterPerformance(t *testing.T, o1 orm.Orm, localProvider provider.Provider) {
	...
	equalFilter, err := localProvider.GetModelFilter(simplePerfModel, model.OriginView)
	if err != nil {
		t.Errorf("GetModelFilter failed, err:%s", err.Error())
		return
	}
	...
	rangeFilter, err := localProvider.GetEntityFilter(&SimplePerf{}, model.OriginView)
	if err != nil {
	...
	likeFilter, err := localProvider.GetModelFilter(simplePerfModel, model.OriginView)
	if err != nil {
		t.Errorf("GetModelFilter failed, err:%s", err.Error())
		return
	}
	...
	complexFilter, err := localProvider.GetEntityFilter(&SimplePerf{}, model.OriginView)
	if err != nil {
		t.Errorf("GetEntityFilter failed, err:%s", err.Error())
		return
	}
	...
	for pageSize := 10; pageSize <= 100; pageSize *= 10 {
		for pageIndex := 0; pageIndex < 3; pageIndex++ {
			...
			pageFilter, err := localProvider.GetModelFilter(simplePerfModel, model.OriginView)
			if err != nil {
				t.Errorf("GetModelFilter failed, err:%s", err.Error())
				return
			}
...
func cleanupPerformanceTest(t *testing.T, o1 orm.Orm, localProvider provider.Provider) {
	// 删除复杂对象
	complexPerfModel, _ := localProvider.GetEntityModel(&ComplexPerf{})
	complexFilter, err := localProvider.GetModelFilter(complexPerfModel, model.OriginView)
	if err != nil {
		t.Errorf("GetModelFilter failed, err:%s", err.Error())
		return
	}
	...
	// 删除简单对象
	simplePerfModel, _ := localProvider.GetEntityModel(&SimplePerf{})
	simpleFilter, err := localProvider.GetModelFilter(simplePerfModel, model.OriginView)
	if err != nil {
		t.Errorf("GetModelFilter failed, err:%s", err.Error())
		return
	}
