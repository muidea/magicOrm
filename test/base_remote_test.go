...
func TestRemoteQuery(t *testing.T) {
	...
	c3 := &Compose{ID: c2.ID, PtrSimple: &Simple{}, PtrSimpleArray: &[]Simple{}, PtrReference: &Reference{}, PtrRefArray: &[]*Reference{}, PtrCompose: &Compose{}}
	c3Value := *c2Value
	err = o1.Insert(&c3Value, remoteOwner)
	if err != nil {
		t.Errorf("insert compose failed, err:%s", err.Error())
		return
	}
	...
	c4 := &Compose{}
	c4Value := *c2Value
	err = o1.Insert(&c4Value, remoteOwner)
	if err != nil {
		t.Errorf("insert compose failed, err:%s", err.Error())
		return
	}

	err = remote.UpdateEntity(&c4Value, c4)
	if err != nil {
		t.Errorf("updateEntity failed, err:%s", err.Error())
		return
	}
	...
	err = o1.BatchQuery(cListValue, nil, remoteOwner)
	if err != nil {
		t.Errorf("batch query compose failed, err:%s", err.Error())
		return
	}
	err = remote.UpdateSliceEntity(cListValue, &cList)
	if err != nil {
