...
func TestTypeConversionsRaw(t *testing.T) {
	...
	intVal := int64(42)
	str, err := ConvertRawToString(intVal)
	if err != nil || str != "42" {
	...
	strVal := "42"
	intResult, err := ConvertRawToInt64(strVal)
	...
	floatVal := 42.5
	floatStr, err := ConvertRawToString(floatVal)
	if err != nil {
	...
	nonBoolStr := "notabool"
	boolVal, err := ConvertToBool(reflect.ValueOf(nonBoolStr))
	if err != nil {
	...
	_, err = ConvertToBool(reflect.ValueOf(complexVal))
	if err == nil {
