...
func (s *Builder) BuildQuery(filter model.Filter) (ret *Result, err *cd.Result) {
	...
	querySQL := fmt.Sprintf("SELECT %s FROM `%s`", namesVal, s.buildCodec.BuildModelTableName(s.hostModel))
	if filter != nil {
...
func (s *Builder) BuildQueryRelation(vField model.Field, rModel model.Model) (ret *Result, err *cd.Result) {
	...
	if relationErr != nil {
		err = relationErr
		log.Errorf("BuildQueryRelation %s failed, s.buildCodec.BuildRelationTableName error:%s", vField.GetName(), err.Error())
		return
	}
