...
func buildVMIProductFilter(t *testing.T) (provider.Provider, database.Builder, *remote.Object, *remote.ObjectFilter, *remote.SliceObjectValue, *remote.ObjectValue) {
	t.Helper()
...
func buildVMIProductValueModel(t *testing.T) (provider.Provider, database.Builder, *remote.Object) {
	t.Helper()
...
func TestBuilderVMIBuildQueryRelationFilters(t *testing.T) {
	...
	if !strings.Contains(sql, "`id` IN (SELECT DISTINCT(`left`) `id`  FROM `tenant_ProductSkuInfo2SkuInfo` WHERE `right` in (?,?))") {
		t.Fatalf("missing skuInfo relation filter in sql: %s", sql)
	}
	if !strings.Contains(sql, "`id` IN (SELECT DISTINCT(`left`) `id`  FROM `tenant_ProductStatus3Status` WHERE `right` = ?)") {
...
func TestBuilderVMINotInRelationFilter(t *testing.T) {
	...
	sql := result.SQL()
	if !strings.Contains(sql, "`id` IN (SELECT DISTINCT(`left`) `id`  FROM `tenant_ProductSkuInfo2SkuInfo` WHERE `right` not in (?,?))") {
...
func TestBuilderVMIQueryAndDeleteRelations(t *testing.T) {
	...
	if queryRelation.SQL() != "SELECT `right` FROM `tenant_ProductSkuInfo2SkuInfo` WHERE `left`= ?" {
	...
	if deleteRelationByRights.SQL() != "DELETE FROM `tenant_ProductSkuInfo2SkuInfo` WHERE `left`=? AND `right` IN (?,?)" {
	...
	if delHost.SQL() != "DELETE FROM `tenant_Status` WHERE `id` IN (SELECT `right` FROM `tenant_ProductStatus3Status` WHERE `left`=?)" {
		t.Fatalf("unexpected delete host sql: %s", delHost.SQL())
	}
	if delRelation.SQL() != "DELETE FROM `tenant_ProductStatus3Status` WHERE `left`=?" {
...
func TestBuilderVMICreateInsertAndDropRelations(t *testing.T) {
	...
	if insertRelation.SQL() != "INSERT INTO `tenant_ProductSkuInfo2SkuInfo` (`left`, `right`) VALUES (?,?)" {
	...
	if dropRelation.SQL() != "DROP TABLE IF EXISTS `tenant_ProductSkuInfo2SkuInfo`" {
...
func TestBuilderVMIMainTableSQL(t *testing.T) {
	...
	if !strings.Contains(createSQL, "`id` BIGINT NOT NULL AUTO_INCREMENT") {
		t.Fatalf("create table should use auto increment primary key: %s", createSQL)
	}
	...
	if !strings.Contains(insertSQL, "INSERT INTO `tenant_Product` (`name`,`description`,`image`,`expire`,`tags`,`creater`,`createTime`,`namespace`) VALUES (?,?,?,?,?,?,?,?)") {
	...
	if !strings.Contains(updateSQL, "UPDATE `tenant_Product` SET `name` = ?,`description` = ?,`image` = ?,`expire` = ?,`tags` = ?,`creater` = ?,`createTime` = ?,`namespace` = ? WHERE `id` = ?") {
	...
	if deleteResult.SQL() != "DELETE FROM `tenant_Product` WHERE `id` = ?" {
