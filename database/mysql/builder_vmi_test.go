...
func buildVMIOrderFilter(t *testing.T) (provider.Provider, database.Builder, *remote.Object, *remote.ObjectFilter, *remote.SliceObjectValue, *remote.ObjectValue) {
	t.Helper()
...
func TestBuilderVMIBuildQueryRelationFilters(t *testing.T) {
	...
	if !strings.Contains(sql, "`id` IN (SELECT DISTINCT(`left`) `id`  FROM `tenant_OrderGoods2GoodsItem` WHERE `right` in (?,?))") {
		t.Fatalf("missing goods relation filter in sql: %s", sql)
	}
	if !strings.Contains(sql, "`id` IN (SELECT DISTINCT(`left`) `id`  FROM `tenant_OrderStatus3Status` WHERE `right` = ?)") {
...
func TestBuilderVMIBuildBatchQueryRelation(t *testing.T) {
	...
	sql := result.SQL()
	if !strings.Contains(sql, "SELECT `left`,`right` FROM `tenant_OrderGoods2GoodsItem` WHERE `left` IN (?,?)") {
...
func TestBuilderVMINotInRelationFilter(t *testing.T) {
	...
	sql := result.SQL()
	if !strings.Contains(sql, "`id` IN (SELECT DISTINCT(`left`) `id`  FROM `tenant_OrderGoods2GoodsItem` WHERE `right` not in (?,?))") {
...
func TestBuilderVMIQueryAndDeleteRelations(t *testing.T) {
	...
	if queryRelation.SQL() != "SELECT `right` FROM `tenant_OrderGoods2GoodsItem` WHERE `left`= ?" {
	...
	if deleteRelationByRights.SQL() != "DELETE FROM `tenant_OrderGoods2GoodsItem` WHERE `left`=? AND `right` IN (?,?)" {
	...
	if delHost.SQL() != "DELETE FROM `tenant_Status` WHERE `id` IN (SELECT `right` FROM `tenant_OrderStatus3Status` WHERE `left`=?)" {
		t.Fatalf("unexpected delete host sql: %s", delHost.SQL())
	}
	if delRelation.SQL() != "DELETE FROM `tenant_OrderStatus3Status` WHERE `left`=?" {
...
func TestBuilderVMICreateInsertAndDropRelations(t *testing.T) {
	...
	if insertRelation.SQL() != "INSERT INTO `tenant_OrderGoods2GoodsItem` (`left`, `right`) VALUES (?,?)" {
	...
	if dropRelation.SQL() != "DROP TABLE IF EXISTS `tenant_OrderGoods2GoodsItem`" {
...
func TestBuilderVMIMainTableSQL(t *testing.T) {
	...
	if !strings.Contains(insertSQL, "INSERT INTO `tenant_Product` (`name`,`description`,`image`,`expire`,`tags`,`creater`,`createTime`,`modifyTime`,`namespace`) VALUES (?,?,?,?,?,?,?,?,?)") {
